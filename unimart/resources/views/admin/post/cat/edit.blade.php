                ...
                <div class="card-body">
                    <form action="{{route("admin.post.cat.update",$post_cat->id)}}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="slug" id="slug" value="{{$post_cat->slug}}">
                        </div>
                        <div class="form-group">
                            <label for="">Danh mục cha</label>
                            <select class="form-control" id="" name="parent_id">
                                <option value="{{0}}" {{$post_cat->parent_id==0?"selected='selected'":""}}>Chọn danh mục</option>
                                @php
                                    $post_cats=array();
                                ...
                                @foreach($post_cats as $k=>$value)
                                    <option value="{{$k}}" {{$post_cat->parent_id==$k?"selected='selected'":""}}>{{$value['name']}}</option>
                                @endforeach
                            </select>
                        </div>
