                    ...
                    <form action="{{route("admin.product.cat.update",$product_cat->id)}}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="slug" id="slug" value="{{$product_cat->slug}}">
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="text" name="position" id="position" value={{$product_cat->position}}>
                                    @error('position')
                                    <small class="text-danger">{{$message}}</small>
                                     @enderror
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="" name="parent_cat">
                                <option value="{{0}}" {{$product_cat->parent_id==0?"selected='selected'":""}}>Chọn danh mục</option>
                               
                                @foreach($menu_1 as $k=>$value)
                                    <option value="{{$k}}" {{$product_cat->parent_id==$k?"selected='selected'":""}}>{{$value['name']}}</option>
                                @endforeach
                            </select>
                        </div>
